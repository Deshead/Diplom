...
Если запрос принимает поля в JSON, передавайте объект, например `{"items": [...]}`. Отдельный список, число, строка или `null` вместо объекта вернут 400.
...
Вход возвращает `{"Status": true, "Token": "..."}`. До подтверждения email вход закрыт. Токен регистрации действует 24 часа, сброса пароля — один час. После смены пароля через API или админку старый токен входа и ранее выданные коды сброса удаляются. Обычная правка профиля их не отменяет. Email и роль нельзя менять через профиль.
...
Товары заблокированного поставщика и магазина, который отключил прием заказов, скрыты из каталога. Их карточки возвращают 404. Категория остается в списке, если она есть хотя бы у одного действующего магазина.
...
У магазина сохраняется источник последнего успешного импорта: URL или имя загруженного файла без пути. Он виден в админке. Неудачный импорт прежний источник не меняет. Текущее состояние приема заказов сохраняется и при обновлении прайса.
