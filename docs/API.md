...
Вход возвращает `{"Status": true, "Token": "..."}`. До подтверждения email вход закрыт. Токен регистрации действует 24 часа, сброса пароля — один час. После смены пароля старый токен входа и ранее выданные коды сброса удаляются. Обычная правка профиля их не отменяет. Email и роль нельзя менять через профиль.
...
У магазина сохраняется источник последнего успешного импорта: URL или имя загруженного файла без пути. Он виден в админке. Неудачный импорт прежний источник не меняет.
