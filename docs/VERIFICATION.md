...
Последняя локальная проверка: 22 сентября 2026 года. Windows, Python 3.14.6, Django 5.2.17, DRF 3.17.2, Celery 5.6.3.

| Проверка | Результат |
| --- | --- |
| Django check и миграции | Ошибок и незаписанных миграций нет |
| `pip check`, Ruff | Ошибок нет |
| SQLite, проверки профиля, каталога и заказов | 57 тестов прошли, один тест блокировок пропущен |
| PostgreSQL 17.11, полный набор | 59 тестов прошли без пропусков |
| Покрытие | 91% |
...
| GitHub Actions | Python 3.10 и 3.12 с SQLite, Python 3.12 с PostgreSQL 17; [результаты запусков](https://github.com/Deshead/Diplom/actions/workflows/tests.yml) |
...
22 сентября прошли 59 тестов на PostgreSQL и оба HTTP-сценария на временных базах SQLite.

Добавлены восемь тестов: старый код сброса не действует после смены пароля; запрос с устаревшим профилем не возвращает прежний пароль; заблокированный поставщик не виден в каталоге; неверный JSON возвращает 400 и не меняет данные. Обычная правка профиля сохраняет возможность входа и действующий код сброса.
